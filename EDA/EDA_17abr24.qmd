...
ggplot(pob_casada, aes(x = as.factor(Año), y = Cantidad, fill = Sexo)) +
  ...
  ggtitle(str_wrap("Casados o en unión libre", width = 50)) +  # Center the title
  theme(
    plot.title = element_text(size = 14, color = "lightgray", hjust = 0.5),  # Set plot title color to lightgray
    plot.background = element_rect(fill = "transparent"),  # Set plot background color to transparent
    panel.background = element_rect(fill = "transparent"),  # Set panel background color to transparent
    legend.background = element_rect(fill = "transparent"),  # Set legend background color to transparent
